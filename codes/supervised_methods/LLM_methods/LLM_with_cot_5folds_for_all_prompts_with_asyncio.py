...
import asyncio
import threading  # Only keep progress bar lock, no longer used for LLM calls

import numpy as np
import pandas as pd
from tqdm.asyncio import tqdm_asyncio  # Asynchronous progress bar
from tqdm import tqdm  # New: Synchronous progress bar
from requests.exceptions import RequestException, Timeout
from tenacity import (
    retry, stop_after_attempt, wait_exponential,
    retry_if_exception_type, AsyncRetrying,  # Asynchronous retry
)  # Synchronous retry included in tenacity base import
...
# Production-level call configuration (pure async version)
ASYNC_TIMEOUT = 300  # Total timeout for async tasks (replace original THREAD_TIMEOUT+API_TIMEOUT)
RETRY_ATTEMPTS = 3  # Retry attempts
RETRY_WAIT = (1, 5)  # Retry exponential backoff
semaphore = asyncio.Semaphore(5)  # Async rate limiting (effective)

# Other configurations (unchanged, keep Chinese in path/filename)
...
MODEL_LIST = ["glm-4.5-flash", "glm-4.6", "qwen-plus", "qwen-flash", "deepseek-reasoner"]
# New: Whether each model supports async, one-to-one correspondence with MODEL_LIST (users can adjust according to reality)
MODEL_ASYNC_SUPPORT = [False, False, True, True, True]
...
try:
    ...
    prompt_config = {
        "task": prompt_rows[0],  # Column 1: Core task description (keep Chinese from xlsx)
        "description": prompt_rows[1],  # Column 2: Aspect description explanation (keep Chinese from xlsx)
        "rules": prompt_rows[2],  # Column 3: Label rule explanation (keep Chinese from xlsx)
        "few_shot": prompt_rows[3]  # Column 4: Example explanation (keep Chinese from xlsx)
    }
...
# ===================== Core Utility Functions (unchanged, English comments) =====================
def extract_last_int_list(text: str) -> List[int]:
    """Extract the last integer list from text (supports [1, -1, 0] format)"""
...
def get_prompt_template(has_description: bool, has_few_shot: bool) -> PromptTemplate:
    ...
    :param has_description: Whether to include aspect descriptions
...
# ===================== Refactoring: Async + Sync LLM Calls (Add sync version) =====================
async def invoke_llm_safe_with_retry(
        llm: ChatOpenAI,
        prompt_template: PromptTemplate,
        review: str,
        review_idx: int,  # New: Review index
        aspects: List[str],
        has_description: bool,
        aspects_and_explanations: Dict[str, str] = None
) -> Tuple[List[int], float]:
    """Asynchronous LLM call: adapt to dynamic unmentioned labels + print failed review index"""
        ...
        async for attempt in AsyncRetrying(
            ...
            with attempt:
                try:
                    ...
                    # Fallback: fill with unmentioned label when tag length is insufficient (core modification)
...
def invoke_llm_safe_with_retry_sync(
    ...
    """Synchronous LLM call: adapt to dynamic unmentioned labels + print failed review index (fix fallback logic)"""
    start_time_single = time.time()
    # Construct input data
    input_data = {
        'review': review,
    ...
    def _sync_invoke():
        try:
            ...
            resp_text = resp.content
            tags = extract_last_int_list(resp_text)[:len(aspects)]
            # Fallback: fill with unmentioned label when tag length is insufficient (fix: dynamically generated, no hardcoding)
            tags = tags if len(tags) == len(aspects) else [unmentioned_label] * len(aspects)
            return tags
        except TimeoutError:
            print(f"\n⚠️ Timeout Warning: Review index {review_idx} processing exceeded {ASYNC_TIMEOUT} seconds | Review snippet: {review[:50]}...")
    ...
    try:
        ...
        traceback.print_exc()
        tags = [unmentioned_label] * len(aspects)  # Fix: dynamically generate fallback labels

    elapsed_single = time.time() - start_time_single
    return tags, elapsed_single


# ===================== Single Fold Training Function (Adapt to Async/Sync Models) =====================
async def train_single_fold(fold_num: int, file_path: str) -> Tuple[List[Dict], List[Dict]]:
    print(f"\n{'=' * 80}")
    ...
    try:
        ...
        print(f"❌ Failed to read Fold {fold_num} data: {str(e)}")
        return [], []
    ...
    for exp_config in EXPERIMENT_CONFIGS:
        ...
        # Create prompt template in advance (reusable)
        prompt_template = get_prompt_template(has_description, has_few_shot)

        # Iterate over models (add index to match async support list)
        for model_idx, model_name in enumerate(MODEL_LIST):
            ...
            try:
                if model_name.startswith("glm"):
                    llm = ChatOpenAI(
                        ...
                        timeout=ASYNC_TIMEOUT  # Add LLM-level timeout
                    )
            ...
            single_times = [r[1] for r in results]  # Actual time consumption of each review (not affected by parallelism)
            ...
            single_time_mean = np.mean(single_times)  # Average time per sample
            single_time_std = np.std(single_times)  # Standard deviation of time per sample
            single_time_min = np.min(single_times)  # Minimum time per sample
            single_time_max = np.max(single_times)  # Maximum time per sample

            # Print results (add async support annotation)
            print(f"\n📊 Fold {fold_num} | {exp_name} | {model_name} Results:")
            print(
                f"Accuracy: {metrics['accuracy']:.4f} | Macro F1: {metrics['f1_macro']:.4f} | Micro F1: {metrics['f1_micro']:.4f}")
            if is_async_supported:
                print(f"Parallel Avg Time per Sample: {avg_time_per_sample_parallel:.2f} seconds/sample")
            else:
                print(f"Parallel Avg Time per Sample: Async Not Supported")
            print(
                f"Actual Avg Time per Sample: {single_time_mean:.2f} seconds/sample (Std: {single_time_std:.2f}, Min: {single_time_min:.2f}, Max: {single_time_max:.2f})")
            ...
            fold_results.append({
                ...
                'is_async_supported': is_async_supported,  # New: mark async support
                ...
                'avg_time_per_sample_single': single_time_mean,  # Actual mean per sample
                'std_time_per_sample_single': single_time_std,  # Std of time per sample
                'min_time_per_sample_single': single_time_min,  # Min time per sample
                'max_time_per_sample_single': single_time_max  # Max time per sample
            })
...
# ===================== 5-Fold Training Main Function (Changed to Async) =====================
async def train_5folds() -> None:
    ...
    # Exclude parallel time column during group summary (avoid string cannot calculate mean/std)
    ...
    print(f"  1. 5-Fold Detailed Results: Raw metrics for each fold + experiment + model (with async support marker)")
    print(f"  2. Model + Experiment Summary: 5-fold mean + standard deviation (core comparison data)")
    print(f"  3. Label-Level Detailed Metrics: Precision/Recall/F1 for each aspect (with async support marker)")
    print(f"{'=' * 80}")


# ===================== Execution Entry (Async Startup) =====================
if __name__ == "__main__":
    # Fix: asyncio event loop issue for Python 3.8+ on Windows
    if os.name == 'nt':
